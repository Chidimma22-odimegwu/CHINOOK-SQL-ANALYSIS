...
GROUP BY g.GenreId, g.Name, Year(InvoiceDate);
...
TopCustomer AS (
    SELECT TOP 1 CONCAT(c.FirstName,' ',c.LastName) AS Value
    FROM customers c
...
AvgTracksPerInvoice AS (
    SELECT AVG(track_count * 1.0) AS Value  -- Multiply by 1.0 to get decimal avg
    FROM (
